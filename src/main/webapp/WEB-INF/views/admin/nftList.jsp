...
<script type="text/javascript">
   	function changeListNum(se){
   		...
   		location.href='/admin/nftList?typeNum=${type}&listNum='+optionNo;
   	}
	...
	function deleteNft() {
		...
		const numVal = document.getElementById('hiddenNum');
		
		numVal.setAttribute('name', 'nftNum');
		deleteForm.setAttribute('method','post');
		deleteForm.setAttribute('action','/admin/updatedelYn');
		deleteForm.submit();
                	...
                	<c:forEach items="${typeList}" var="typeList">
                		<c:choose>
                			<c:when test="${typeList.typeNum == type}">
                				<li class="active"><a>${typeList.typeName}</a></li>
                			</c:when>
                			<c:otherwise>
                    	...
                    	<div class="tab1">
	                    	<div class="search_btn clearFix">
	                    		<select onchange="changePlaceHolder(this)" id="selectedSearchType">
	                    			...
	                    			<option value="searchId">NFT ID</option>
	                    			<option value="searchYear">원본 게시연도</option>
	                    			<option value="searchStatus">상태</option>
	                    		</select>
	                    		<div id="searchCase">
	                    			<div class="search_icon_title">
							...
							<div class="tblwrap_basic tblwrap_basic_center">
								<table>
									...
									<tbody>
										...
										<c:forEach items="${nftList}" var="list">
								...
								<div class="page_nation">
									<c:choose>
										<c:when test="${beginPage <= 1}">
											<a class="page_first" href="${pageContext.request.contextPath}/admin/nftList?typeNum=${type}&pageNum=1&listNum=${listNum}">
										...
										<c:otherwise>
											...
											<a class="page_prev" href="${pageContext.request.contextPath}/admin/nftList?typeNum=${type}&pageNum=${beginPage-1}&listNum=${listNum}">
												<img src="${pageContext.request.contextPath}/resources/images/arrow_left.png" alt="이전페이지로">
											</a>
										</c:otherwise>
									</c:choose>
									<c:forEach begin="${beginPage}" end="${endPage}" var="page">
										<c:choose>
											...
											<c:otherwise>
												<a class="pageNum" href="${pageContext.request.contextPath}/admin/nftList?typeNum=${type}&pageNum=${page}&listNum=${listNum}" class="">${page}</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									<c:choose>
										<c:when test="${endPage >= totalPageCount}">
			                                ...
			                                <a class="page_last" href="${pageContext.request.contextPath}/admin/nftList?typeNum=${type}&pageNum=${totalPageCount}&listNum=${listNum}">
			                                	<img src="${pageContext.request.contextPath}/resources/images/arrow_last.png" alt="마지막페이지로">
			                                </a>
										</c:when>
										<c:otherwise>
											<a class="page_next" href="${pageContext.request.contextPath}/admin/nftList?typeNum=${type}&pageNum=${endPage+1}&listNum=${listNum}">
			                                	<img src="${pageContext.request.contextPath}/resources/images/arrow_right.png" alt="다음페이지로">
			                                </a>
			                                <a class="page_last" href="${pageContext.request.contextPath}/admin/nftList?typeNum=${type}&pageNum=${totalPageCount}&listNum=${listNum}">
			                                	<img src="${pageContext.request.contextPath}/resources/images/arrow_last.png" alt="마지막페이지로">
