...
public class SendBurnNftId2BlockchainApi {
...
//				.url("http://121.78.145.25:4000/NFT_Burn").post(body).build(); // SK_배포용
		...
		try {
	        ...
	        if (responseMap != null && responseMap.get("success").toString().equals("true")) {
	        	// logger.info("Burn_NFT API Result : "+responseMap.get("success").toString());
	        	new MessageRedirectUtil().redirect("삭제가 완료되었습니다.", "/admin/nftList");
			} else {
				if (!responseMap.get("success").toString().equals("true")) {
					logger.info("Burn_NFT API Result : false");
					throw new IOException();
				}else {
					logger.info("Burn_NFT API - 리턴값 없음");
					throw new IOException();
				}
			}
			...
			// e.printStackTrace();
			logger.info("Burn_NFT API - 응답 없음");
			new MessageRedirectUtil().redirect("NFT 삭제 중 오류가 발생하였습니다.", "/admin/nftList");
		}
